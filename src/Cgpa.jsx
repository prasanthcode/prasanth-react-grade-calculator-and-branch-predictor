...
import credits from './btech_credits.json';
...
const handleSgpaChange = (index, value) => {
  ...
  setSgpa(updated);
    ...
    const [weightedSgpas, setWeightedSgpas] = useState(Array(sems.length).fill(0));
const handleCgpaChange = () => {
  ...
  return (
    <div style={{textAlign:"right",padding:"0 40px"}} className='cgpa_container'>
      <span className="new">New </span>
          <h1 className='cgp' id="totalcgpa">{cgpa}</h1>
          <h3 style={{width:"100%",textAlign:"center" }}>CGPA Calculator</h3>
<span style={{paddingRight:"10px"}}>

Please select your branch 
</span>
    <select name="branch" value={branch} onChange={(e) => handleBranch(e)} className='branchoption'>
...
<p style={{width:"100%",textAlign:"center"}}>Enter SGPA only for the semesters you want to calculate CGPA for</p>
 ...
 <input
      ...
      />
                  ...
                  <td>{sgpa[index]!=="" ? weightedSgpas[index].toFixed(2):"0.00"}</td>
                </tr>
             ))
          ...
          <button onClick={handleCgpaChange} className="btn" >Calculate </button>
    </div>
  )
}
